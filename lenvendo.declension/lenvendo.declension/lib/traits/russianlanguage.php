...
trait RussianLanguage
{
    ...
    {
        return in_array(MultibyteString::lower($consonant), ['ж', 'ш', 'ч', 'щ'], true);
    }
    ...
    {
        return in_array(MultibyteString::lower($consonant), ['г', 'к', 'х'], true);
    }
    ...
    {
        return MultibyteString::countChars($string, static::$vowels);
    }
    ...
    {
        $consonant = MultibyteString::lower($consonant);
    ...
    {
        if (($substring = MultibyteString::findLastPositionForOneOfChars(MultibyteString::lower($word), static::$consonants)) !== false) {
            if (in_array(MultibyteString::slice($substring, 0, 1), ['й', 'ч', 'щ', 'ш'], true)) { // always soft consonants
                return true;
            }

            if (MultibyteString::length($substring) > 1 && in_array(MultibyteString::slice($substring, 1, 2), ['е', 'ё', 'и', 'ю', 'я', 'ь'], true)) { // consonants are soft if they are trailed with these vowels
                return true;
            }
        }
        return false;
    ...
    public static function checkBaseLastConsonantSoftness($word)
    {
        ...
        if (($substring = MultibyteString::findLastPositionForOneOfChars(MultibyteString::lower($word), $consonants)) !== false) {
            if (in_array(MultibyteString::slice($substring, 0, 1), ['й', 'ч', 'щ', 'ш'], true)) { // always soft consonants
                return true;
            }
    ...
    {
        return in_array(MultibyteString::lower($vowel), ['е', 'ё', 'ю', 'я'], true);
    }
    ...
    {
        if (in_array(MultibyteString::lower(MultibyteString::slice($word, 0, 1)), ['а', 'о', 'и', 'у', 'э'], true)) {
    ...
    {
        $verb = MultibyteString::lower($verb);
        // возвратный глагол
        if (MultibyteString::slice($verb, -2) === 'ся') {

            return ($gender == Gender::MALE
                ? $verb
                : MultibyteString::slice($verb, 0, -2).(MultibyteString::slice($verb, -3, -2) === 'л' ? null : 'л').'ась');
        }

        // обычный глагол
        return ($gender == Gender::MALE
            ? $verb
            : $verb.(MultibyteString::slice($verb, -1) === 'л' ? null : 'л').'а');
    }
    ...
    {
        $normalized = trim(MultibyteString::lower($word));
        if (in_array(MultibyteString::slice($normalized, 0, 1), ['в', 'ф'], true))
            return 'во '.$word;
        return 'в '.$word;
    }
    ...
    {
        $normalized = trim(MultibyteString::lower($word));
        if (in_array(MultibyteString::slice($normalized, 0, 1), ['c', 'з', 'ш', 'ж'], true) && static::isConsonant(MultibyteString::slice($normalized, 1, 2)) || MultibyteString::slice($normalized, 0, 1) === 'щ')
            return 'со '.$word;
        return 'с '.$word;
    }
    ...
    {
        $normalized = trim(MultibyteString::lower($word));
        if (static::isVowel(MultibyteString::slice($normalized, 0, 1)) && !in_array(MultibyteString::slice($normalized, 0, 1), ['е', 'ё', 'ю', 'я'], true))
            return 'об '.$word;

        if (in_array(MultibyteString::slice($normalized, 0, 3), ['все', 'всё', 'всю', 'что', 'мне'], true))
            return 'обо '.$word;
    ...
    {
        return in_array(MultibyteString::slice($noun, -2), ['ой', 'ий', 'ый', 'ая', 'ое', 'ее'])
    ...
    {
        if ($animate) {
            return $forms[RussianCases::RODIT];
        }

        return $forms[RussianCases::IMENIT];
    }
}
